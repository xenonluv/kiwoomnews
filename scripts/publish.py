...
from team2_relevance import material_freshness_view, parse_evidence_datetime  # noqa: E402
...
def refresh_material_freshness(out):
    """게시·로컬·history가 동일한 신호시각 재료 신선도를 갖게 한다."""
    as_of = parse_evidence_datetime(out.get("generated_at"))
    if as_of is None:
        return out
    for row in list(out.get("suspects", [])) + list(out.get("blocked_suspects", [])):
        if isinstance(row.get("material"), dict):
            row["material"] = material_freshness_view(row["material"], as_of=as_of)
    return out
...
def record_history(out):
    ...
    generated_date = "".join(ch for ch in str(out.get("generated_at") or "") if ch.isdigit())[:8]
    historical_republish = bool(generated_date and generated_date != today)
    ...
    for rank, s in enumerate(out.get("suspects", []), 1):
        ...
        hist["suspects"][s["code"]] = {
            ...
            "material": (prev.get("material") if historical_republish and prev.get("material")
                         else s.get("material")),  # 장전 재게시로 신호시점 재료를 미래 값으로 덮지 않음
            ...
            "market_alert_snapshot": (
                prev.get("market_alert_snapshot") if historical_republish
                and prev.get("market_alert_snapshot") is not None
                else s.get("market_alert_snapshot")),
            ...
            "alert_risk_release_evidence": (
                prev.get("alert_risk_release_evidence") if historical_republish
                and prev.get("alert_risk_release_evidence") is not None
                else s.get("alert_risk_release_evidence")),
            ...
            "evaluation_status": prev.get("evaluation_status"),
            "evaluation_exclusion": prev.get("evaluation_exclusion"),
    ...
    for s in out.get("blocked_suspects", []):
        ...
        blocked_hist[code] = {
            ...
            "price": s.get("price"),
            "pattern": s.get("pattern"),
            "material": (previous.get("material") if historical_republish
                         and previous.get("material") else s.get("material")),
            "rank_bucket": s.get("rank_bucket"),
            "suspicion_score": s.get("suspicion_score"),
            "market_alert_snapshot": (
                previous.get("market_alert_snapshot") if historical_republish
                and previous.get("market_alert_snapshot") is not None
                else s.get("market_alert_snapshot")),
