...
# 💥 흔들기 순위: 회전율 스윗스팟(90~120%)이 익일 상승 탄력 최강, 과회전(>150%)은 물량 소진(회장님 20년 검증).
# 실증 2026-07-06: 진흥기업 회전 99%→상한가 / 삼기 회전 233%→+8.5%. 게이트 불변, 순위에만 반영.
SHAKEOUT_TURNOVER_SWEET_LO = 90.0
SHAKEOUT_TURNOVER_SWEET_HI = 120.0
SHAKEOUT_TURNOVER_OVERHEAT = 150.0


def _shakeout_turnover_tier(x):
    """흔들기 회전율 순위 tier(낮을수록 우선): 0=스윗스팟 90~120 / 1=허용(40~90·120~150) / 2=과회전>150(강등)."""
    t = x.get("turnover_pct") or 0
    if SHAKEOUT_TURNOVER_SWEET_LO <= t <= SHAKEOUT_TURNOVER_SWEET_HI:
        return 0
    if t > SHAKEOUT_TURNOVER_OVERHEAT:
        return 2
    return 1
...
def scan_shakeout(p):
    ...
    for row in cand:
        ...
        out.append({
            ...
            "turnover_band": _shakeout_turnover_tier({"turnover_pct": turnover}),  # 0=스윗90~120 1=허용 2=과회전(검증축)
