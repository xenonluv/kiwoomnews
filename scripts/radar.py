...
파이프라인:
  ...
  [식음(중간)] 신호일(오늘) 현재가 ≥ MA20 생존 + 폭발 전후 창에서 투신 순매수(매집).
   ▼
  [재반등(오늘)] 당일 고가 등락률 −4~+10% + 10분봉 몸통%≥2%(당일 한 번이라도)
...
REACCUM_HIGH_MIN = -4.0            # 당일 고가 등락률 하한(%) — 폭등이 아닌 "살짝 반등"
REACCUM_HIGH_MAX = 10.0           # 당일 고가 등락률 상한(%)
...
def scan_reaccum_candidate(rec, p, events):
    ...
    if not now.get("price") or not now.get("prev_close"):
        return None
    # 재반등 게이트 ①: 당일 고가 등락률 [−4,+10] (폭등이 아닌 "살짝 반등 초입").
    # 현재 등락률도 같은 밴드 내여야 함 — 고가만 잠깐 났다가 폭락 중(예: 고가0% 현재-14%)인
    # 종목이 '재반등'으로 잡히는 것을 막는다(현재가 ≤ 고가라 상한은 사실상 high에 종속).
    high_pct = (now["high"] / now["prev_close"] - 1) * 100 if now.get("high") else None
    if high_pct is None or not (p.reaccum_high_min <= high_pct <= p.reaccum_high_max):
        return None
    if not (p.reaccum_high_min <= now["change_pct"] <= p.reaccum_high_max):
        return None
    try:
...
def main():
    ...
    ap.add_argument("--reaccum-high-min", type=float, default=REACCUM_HIGH_MIN,
                    help="당일 고가 등락률 하한(%%, 기본 -4)")
    ap.add_argument("--reaccum-high-max", type=float, default=REACCUM_HIGH_MAX,
                    help="당일 고가 등락률 상한(%%, 기본 +10)")
    ...
    out = {
        ...
        "params": {"reaccum_high_range": [p.reaccum_high_min, p.reaccum_high_max],
