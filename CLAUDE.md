...
- `/alpha` — **알파 전진검증** 페이지(SSG + 60초 폴링). 데이터 = `alpha.json`. mover 카드(유통회전율 2일·종가강도·14:30스파크·
  ...
  - **14:30스파크 강도 색·정렬**(웹 표시만, alpha.json 무수정): 카드 정렬=스파크 큰 순(동순위 2일회전율, 미측정 맨뒤). 색=한국관례(빨강=강) 5+빨강·굵게/3~4주황/1~2앰버/0·미측정 회색. `spark_source="none"`은 "— 미측정"(0과 구분).
  - **🕵 키움 속 숨은 외국인 매집 배지**(`hiddenForeign`): 투자자별 **외인 순매수(+) AND 외국계창구 순매수<외인×10% AND 키움 매수집중≥30%** → 외국인이 외국계 창구를 안 거치고 키움 등 리테일 창구로 숨어 매집한 흔적(의심). 외인 순매수 규모로 보라 강도색(10만+진한/3만+중간/그외 연한). **"창구≠주체" 한계 명시(툴팁)·거래원/투자자 결측 시 미표시**(날조 방지). 표시 전용(통계 무관).
...
- 알파 UI: `components/alpha/AlphaList.tsx`(/alpha 전진검증 mover 카드 + CalibrationPanel). 60초 폴링·SSG. **agent_alpha 사이드카**
  ...
  스파크 강도색·큰 수 우선 정렬(`sparkClass`/`sparkRank`) + 키움 속 숨은 외국인 매집 보라 배지(`hiddenForeign`/`HF_BADGE`) — 위 `/alpha` API 항목 참조. 모두 웹 표시 전용(alpha.json·정렬 데이터 무수정).
