...
- **KIS 공식 API** (`openapi.koreainvestment.com:9443`, .env의 KIS_APP_KEY/SECRET):
  ...
  - ⚠️ **시장구분(`FID_COND_MRKT_DIV_CODE`) 분리 정책 — 가격=J / 거래대금·수급=UN**:
    ...
    매집액은 UN/J≈1.0**(투신은 KRX 전용) → flow 점수 불변. **분봉(reignition)도 UN 전환 완료(2026-06-22 장중
    실측)** — `minute_bars_today(market=MONEY_MARKET)`, 정규장 시간창 가드(SESSION_OPEN~CLOSE)로 NXT 장전·
    야간 봉 배제(가드 통과 정규장 봉은 UN==J 실측 — J봉수==UN봉수, OHLC 왜곡 없음). 10분봉 거래대금 r10(UN/J)
    ≈1.5~1.9 → `REIGNITION_VALUE_10M` 30억→**45억**(×1.5), re_value 45~450억 재산정. got==0 조기종료는 페이지
    최소시각 커서로 보강(장 밖 봉만 든 페이지를 건너 정규장 도달). ⚠ `KIS_MARKET=J` 롤백 시 분봉도 J로
    돌아가나 임계(45억·1,500억 등)는 UN 기준 그대로라 게이트가 과엄격 — 롤백은 임계 동반 환원 필요(드문 운영행위).
