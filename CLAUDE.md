...
> 최종 갱신: **2026-07-23 / rank4-v2 확정**(bucket 0 = 강한흔들기+조합D 최상위·Tier1 단독 다음·Tier2 단독 강등, 발효 `20260724`, v1과 forward 분리). 직전: 2026-07-14 추천 전 공시·거래가능여부 선확인 절대규칙(금호전기 사고, `AGENTS.md §0`). 기저: **Mac 로컬 JSON 권위 원본**(관찰 전수·분봉·의사결정·자동매매 감사, prior/retro/forward 분리). SQL/SQLite는 사용하지 않는다.
...
## suspects 정렬 SSOT (2026-07-23 rank4-v2 확정 / 2026-07-10 정렬4 기반)

아래 표가 현재 suspects 화면·자동매매 순위의 단일 기준이다. 탐지 트랙 설명에 남아 있는 예전 "급소 최상단", "저점매집 관찰축", "매우좋음후보 우선" 문구보다 이 표가 우선한다. **모델 SSOT는 `scripts/rank_policy.py`이며 현행은 `rank4-v2`(발효일 `20260724`).** bucket 0만 v2에서 바뀌었고 1~11은 정렬4(v1)와 동일하다.

| bucket | 조건 | 운영 판단 |
|--------|------|-----------|
| 0 | **강한흔들기+조합D**(`very_good` AND `very_good_tier in (tier1, tier2)` AND `strength_tier >= 3`) → 최상위. 그다음 **매우좋음 Tier1 단독**(`very_good` AND `very_good_tier == tier1`, 조합D 아님) | rank4-v2(2026-07-23). 내부 정렬 Tier1+조합D → Tier2+조합D → Tier1 단독. **Tier2 단독(조합D 아님)은 bucket 0에서 강등**돼 자기 흔들기 bucket(4~8)으로 자연 편입(v1의 "Tier2도 bucket 0" 폐지). 저장된 tier 문자열만 믿지 않음 — `very_good=False`면 tier1 문자열이어도 미승격. 강한흔들기+조합D 교집합은 전 시장 n=2 소표본이라 기대확률 하드게이트가 아닌 **관찰우선순위**(baseline은 현행 bucket0 전체 n=34·터치7 73.5%로 재보정) |
...
정렬 내부 tie-breaker: `suspicion_score` 내림차순 → 흔들기 `fade_pct` 내림차순 → `peak_turnover_pct` 내림차순 → `turnover_2d_pct` 내림차순 → 기존 안정 tie-breaker. **bucket 0 내부는 이 tie-breaker보다 먼저 `_very_good_sort_rank`(Tier1+조합D=0 → Tier2+조합D=1 → Tier1 단독=2)로 정렬한다.**

정렬4/rank4-v2 구현 원칙:

- **bucket 0 승격은 `very_good is True`를 반드시 요구한다(rank4-v2).** `_is_very_good_combo_d`(shakeout AND very_good AND tier1/tier2 AND `strength_tier>=3`)가 최상위, `very_good` AND `tier1` 단독이 그다음. `very_good=False`인데 tier 문자열만 tier1/tier2인 불일치 저장값은 승격하지 않고 일반 흔들기 bucket으로 간다.
...
- **버전 분리**: rank4-v2는 발효일 `20260724`부터 forward로 인정한다. rank4-v1(발효 `20260713`) 신호는 v1 forward로 남기고 v2 성과표와 섞지 않는다(`radar_backtest.KNOWN_FORWARD_EFFECTIVE_FROM`). 버전 승격 직후 v2 forward 표본은 n=0에서 시작하므로 킬스위치는 표본이 쌓일 때까지 "수집 중"이다.
...
- `schema.json`, `models/rank4-v1.json`: 스키마와 불변 모델 정의. 모델 SSOT는 `scripts/rank_policy.py`이며 **현행은 `rank4-v2`(발효일 `20260724`)**. rank4-v1(발효 `20260713`)은 20260724 이전 신호의 모델로 보존·평가하고 v2와 forward를 분리한다. 2026-07-10은 `mixed-deployment` 참고값으로만 보존하고 forward에서 제외한다.
...
| `rank_policy.py` | `rank4-v2`(현행, 발효 `20260724`) 버킷·정렬·40년 경험칙 prior·모델 발효일의 코드 SSOT. radar와 backtest가 공용으로 import한다. |
