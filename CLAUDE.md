...
| `publish.py` | radar → `web/data/radar.json` → 변경 시에만 commit+push. flock 락, `--dry-run`, `--max`. radar 인자 그대로 전달. `explosions[]`·`youtong[]`은 radar.py 출력 그대로 패스스루. 매 회차 `data/radar_history/`에 검증용 이력(raw 점수) 기록. 게시 후보 중 **재반등 봉 발생 시 `telegram_notify.notify_reignitions` 호출** + **youtong 진입 시 `notify_youtong` 호출**(각각 별도 try·토큰 없으면 조용히 skip. ⚠ youtong 알림은 **기본 OFF** — 2026-07-02 텔레그램 개편(포착 시점 이미 +20% 고위험군), `TELEGRAM_YOUTONG=1`로 부활. 🚨 재반등 스파크는 유지 — 14:30 이후에만 발생하는 회장님 핵심 신호). |
...
| `notify.py` | 자체 텔레그램 "🧠 [알파]" — 고신뢰(confidence≥0.6) 판단만 종목·일자 1회 디둡. 코어 `telegram_notify.send`/`load_env` 재사용. 기존 🚨/⚡와 분리. ⚠ **기본 OFF**(2026-07-02 텔레그램 개편 — 장중 소음 제거, `ALPHA_LLM_TELEGRAM=1`로 부활). |
| `notify_close_bet.py` | **🎯 [종베] 오늘의 TOP 후보 텔레그램** (2026-07-02 개편의 신설 핵심 알림). publish_alpha.run()이 게시 직후 호출(별도 cron 없음 — 15:15 잠정·15:43 확정 잡 재사용). 오늘 movers를 **웹과 1:1 정렬**(fitness desc·value_eok desc·code)로 TOP2 발송(부적합<45 제외, 전원 부적합이면 "오늘 후보 없음" 1통). **시간게이트 14:55~16:00**(17:47 보정 실행 차단, `CLOSE_BET_FORCE=1` 테스트 우회) + 구성서명 디둡(`data/.close_bet_notified.json`) — 15:43 실행은 TOP 구성이 바뀐 경우만 "🔄 확정 변경" 후속 1통. 폭락제외 발동 수·"익일 +7% 익절/−5% 손절·매수추천 아님" 고정 표기. fail-safe(실패해도 publish 진행). **💥 흔들기 섹션**: radar.json(READ ONLY)의 당일 shakeout 종목을 회전·페이드·익절힌트와 함께 별도 표기(당일 게시분만 스테일 가드·최대 3종 — 회장님 종베 전용 셀, 2026-07-04). |
| `alert_watch.py` | **KRX 시장경보 감시·예측** (회장님 지시 2026-07-03). `alert_now`=네이버 marketAlertType(주의/경고/위험 현재 지정) + `forecast_warning`=KIND 공시 공식(5일 +60%·15일 +100%·3일 +100%·15일 신고가·지수 배수 5x/3x)으로 **마감 직전 "내일 경고예고/지정 뜰지" 계산**(15:10 수집가=가상 종가). 무경보 상태면 "경고예고 예상"으로 캡(KRX 절차). quant가 `alert_now`/`alert_forecast` 저장 → /alpha 카드 대형 배지(⚠️투자주의·🚨경고예고 예상) + 15:15 종베 텔레그램 표기. **2026-07-03부터 점수 반영**: fitness 감점 주의 −10/경고 −30/위험 −60/경고지정예측 −25·해제예정 +20(v4 산식 참조, 배지 병행). 투자주의(1단계)는 계좌 기반 유형이라 예측 불가(현재 상태만). 덕신이피씨 7/2 소급 재현 검증(5일 +62% → 예측 적중). fail-safe. |
| `alert_release.py`(코어 `scripts/`) | **투자경고 지정해제 예측** (회장님 지시 2026-07-03). KRX 해제공식(KIND 공시 원문 실측): 지정일 기산 **10매매일 경과** + 판단일 종가가 **5일전×1.45 미만 AND 15일전×1.75 미만 AND 최근 15일 최고가 아님** → 다음 거래일 해제. 지정일 = 네이버 PC 공시목록(`finance.naver.com news_notice`, EUC-KR)의 '투자경고종목 지정' 공시일 **다음 거래일**(지정중/매매거래/예고/해제 제목 오매칭 필터). 스캔 8p 밖=OLD(경과 간주·가격요건은 유지), 1p부터 파싱실패=None(오폭 차단·fail-safe). radar(경고 수상종목 최상단 승격+`alert_release` 필드)·agent_alpha quant(fitness +20)가 공용 — 코어 모듈이라 격리 무손상(alpha가 읽기전용 import). |
| `publish_alpha.py` | **web 게시(cron 15:15 잠정·15:43 확정·17:47 보정)** — **최신 1거래일(오늘) forward만**(`RECENT_DAYS=1` — 어제·오늘 혼재 방지, 회장님 지시 2026-06-29) + calibration → `web/data/alpha.json`. 익일결과 라벨·검증은 calibrate(forward_samples 전체)가 담당. 변경판정=**git HEAD 커밋본 기준**, pathspec 한정 commit + `GIT_LOCK` 공유 직렬화 + 변경 시에만 push. `--dry-run` 미기록. 게시 직후 `notify_close_bet.notify()` 호출(🎯 종베 TOP 후보 텔레그램 — fail-safe). |
...
- `/alpha` — **알파 전진검증** 페이지(SSG + 60초 폴링). 데이터 = `alpha.json`. mover 카드(유통회전율 2일·종가강도·14:30스파크·
  ...
  + **14:30 스파크 횟수 단독** + **🕵 키움 속 외인매집(해당/미해당)** + **종합점수(스파크+외인매집)별 — 정렬 순위 검증** 실측 익일확률·**평균 익일고가·+7% 익절터치율**(종가베팅→다음날 고가 도달폭), **min_n 게이트**=부족 셀 "관찰중"). **모든 셀에 touch7_rate(+7% 고가터치)·touch13_rate(+13% 고가터치=회장님 실제 익절선·흔들기 고회전 tp_hint) 병기**(2026-07-04 — 검증 잣대를 실제 전략에 정렬). CalibrationPanel 상단에 💥 흔들기 셀(by_mover_type[shakeout]) 전용 요약(레이더 최상단 신호 라이브 전진검증). 종목 익일배지에 종가등락 + **고가 도달폭**(`next_high_pct`) 표시.
  ...
  - **종가베팅 적합도 v4 정렬·색**(산식 SSOT=`agent_alpha/fitness.py:close_bet_fitness`, 웹 미러=`closeBetFitness` — **1:1 동기화 필수**): 카드 정렬=**점수(0~100) 내림차순 · 동점 거래대금(value_eok) 내림차순 · code** (calibrate `by_close_bet_rank`와 동일 키 — 구 |회전2d−115| 타이브레이크는 스윗스팟 유산이라 폐기). 점수=기준 50 + 가감: **유형**(reaccum +10 / youtong 0 / **explosion −50** — 실행성 벌점: 상한가류 종가 체결불가·익일 갭 리스크, 고가터치 통계는 기저급) + **유동성결핍**((대금<50억 OR 2일회전<40%) → **−15 한 번만** — 씨피시스템 함정, 이중처벌 금지) + **거래대금**(≥1000억 +10 — 실증 최강 +13.2%p·LODO 4/4) + **당일등락**(≤−10% 깊은눌림 +15=유일한 실증 가점(터치 100%) / 0~+8% +12 / −10~0 +8 / +8~15 −20=실증 데드존(−41.7%p) / +15~22 −30 / +22%+ −40 — +15%↑는 실행성(추격불가) 근거, 회장님 지시) + **스파크 세기**(최대몸통 0<x<3% '약스파크' **−8** — 최견고 음신호(−16.8%p·LODO 0/4) / **강스파크(3%↑) 2회↑ +8** — 회장님 지시 2026-07-02 "고가를 먹는 프로그램": 과거 4/4 익일 +7% 터치·+21.9%p, 단 종가는 2/4 참사=순수 익절 신호·n=4 소표본 / 강1회·무스파크 0 — 무>강 관측, 서열 판정은 `by_spark_strength`(무/약/강1/강2+) 관찰축 성숙 후. **탐지 게이트(1.5%)와 랭킹 신호(세기)의 임계 분리**) + **강마감**(종가강도≥0.6 −5) + **숨은외인**(lv≥1 −5) + **시장경보 지정**(KRX `alert_now` — **경고 −30 / 위험 −60**, 단 **경고인데 '내일 해제 예정'(`alert_release`)이면 반대로 +20 최대 가산점**(해제=억눌림 해소 재료, 웹 🔓 대형 배지·레이더 정렬 최상단 승격 — 회장님 지시 2026-07-03), **주의 −10·경고지정예측(alert_forecast, 경고/위험 미지정시) −25** — 덕신 7/2 실측(주의+요건충족 무감점 1위 → 익일 −17.8%) 재발 차단 — 회장님 지시 2026-07-03 "투자경고 지정 종목이 85점 1위 추천"(금호건설). 경고 후 재상승=매매정지 지정 리스크, `calibrate.by_alert` 관찰축 전진검증) + **20일선**(현재가 < 일봉 20일선(역배열) **−20** — 회장님 지시 2026-07-03 "적어도 20일선 위". 과거 38표본 전원 위=오폭 0건, 첫 위반자 희림 7/2가 7/3 하락 실증. `ma20_gap_pct`, 이력<20일 신규상장은 결측 통과. `by_ma20` 관찰축 전진검증) + **폭락제외**(과확장붕괴: run_6d≥+100% & 당일음수 −30 / 연속하락 4일↑ −15 — 2026-07-02 회장님 지시 "폭락 중인 종목 상위 차단"(금호건설 5연상붕괴·광주신세계 4일 연속하락 사례). 39표본 오폭 0건 검증, 승자군 최대 연속하락 3일. ⚠ peak_dd(고점 이탈폭)는 승자들이 −26~−47%에서 나와 **점수 미반영·관찰 전용**). 2일회전율 밴드 가감은 전삭제(지그재그=노이즈). 색=75+적합·빨강굵게/60+중간·주황/45+약·앰버/그외 부적합·회색. 순위 배지는 **부적합(<45) 종목이면 1~3위여도 강조색 미적용**(티어 우선·과신 방지), 헤더에 "순위상관≈0 — 하위권 회피용" 명시. ⚠ **2026-07-02 4각도 감사+2인 심판 "수정승인" 판결 반영(39표본·4거래일). 순열검정상 우연통과율 97.9% — 어떤 축도 통계 검증 아님, 재앙회피용 잠정 휴리스틱. ~07/25 표본 성숙 전 튜닝 동결.** 관찰축 `by_value_band`/`by_spark_strength`/`by_liquidity_deficit`가 채점축을 자동 전진검증.
