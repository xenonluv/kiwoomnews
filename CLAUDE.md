...
- **재매집/반등(reaccum 수상종목)** — **최근 6거래일 폭발 종목**(전일 폭발)이 **14:30~장종료 5분봉
  ...
  조용한 매집 구간)인 상태. ⚠️ MA20 생존·투신(ivtr)·거래원·거래대금 게이트는 미사용(전일 폭발 + 마감 직전
  5분 양봉 스파크 + 등락률 범위만). 당일 폭발(signal_date==peak_date)은 `/forecast`에만, 수상종목은 '과거 폭발 + 오늘 재분출'.
...
[정밀 판정·종목별, KIS 공식 API]
   재매집: minute_bars_today → 14:30~장종료 5분봉 양봉(몸통%≥2%) 2회 이상 AND 현재 등락률 −5~+7%. 전일 폭발 종목만.
   ▼
...
- `/youtong` — **곧 폭발할 후보**(위로 올라오며 분출) 페이지(SSG + 60초 폴링). 데이터 = `radar.json`의 `youtong[]`
  (09:30↑·현재 등락률≥7% AND 유통회전율≥50%(상한없음) AND 09:30↑ 5분봉 양봉 스파크≥1, 폭발 제외). **종일 지속**
