...
| `calibrate.py` | **채점·보정(cron 17:45)** — 라벨 표본으로 정량밴드(turnover_2d × 스파크 × 종가강도 × 음봉)별 **실측 익일확률** + LLM Brier → `calibration.json`. **min_n=20 게이트**(부족 셀 "관찰중", 전 셀 보고=체리피킹 금지). 스파크축은 측정행(source≠none)만. 축: `by_turnover2d_eumbong`·`by_spark_eumbong_hi_turnover`·`by_close_strength_eumbong`·**`by_spark_count`(14:30 스파크 횟수 단독·전체)**·**`by_hidden_foreign`(키움 속 외인매집 해당/미해당)**·**`by_combined_score`(합산 종합점수=스파크+외인매집 밴드 — /alpha 정렬 순위 자체의 적중률)**·`cells`. hidden_foreign/combined은 quant 저장값(SSOT) 우선·옛 행만 재계산. **모든 셀 `_stat`에 종가베팅 가정 익일고가 지표: `avg_high`(종가 대비 평균 익일 고가 등락)·`touch7_rate`(익일 +7% 고가 터치율=익절 도달%)**. 신규 축은 데이터 소급(forward에 원천 다 적재됨, label.py가 `next_high_pct` 기록). |
| `notify.py` | 자체 텔레그램 "🧠 [알파]" — 고신뢰(confidence≥0.6) 판단만 종목·일자 1회 디둡. 코어 `telegram_notify.send`/`load_env` 재사용. 기존 🚨/⚡와 분리. |
| `publish_alpha.py` | **web 게시(cron 15:18 잠정·15:43 확정·17:47 보정)** — **최신 1거래일(오늘) forward만**(`RECENT_DAYS=1` — 어제·오늘 혼재 방지, 회장님 지시 2026-06-29) + calibration → `web/data/alpha.json`. 익일결과 라벨·검증은 calibrate(forward_samples 전체)가 담당. 변경판정=**git HEAD 커밋본 기준**, pathspec 한정 commit + `GIT_LOCK` 공유 직렬화 + 변경 시에만 push. `--dry-run` 미기록. |
...
- `/alpha` — **알파 전진검증** 페이지(SSG + 60초 폴링). 데이터 = `alpha.json`. mover 카드(유통회전율 2일·종가강도·14:30스파크·
  ...
  - **14:30스파크 강도 색·정렬**(웹 표시만, alpha.json 무수정): 카드 정렬=**스파크 횟수(미측정 −1) + 키움 속 외인매집 강도(0~3) 합산 큰 순**(둘 다 강한 종목 최상위, 동점 2일회전율). 색=한국관례(빨강=강) 5+빨강·굵게/3~4주황/1~2앰버/0·미측정 회색. `spark_source="none"`은 "— 미측정"(0과 구분).
...
- 알파 UI: `components/alpha/AlphaList.tsx`(/alpha 전진검증 mover 카드 + CalibrationPanel). 60초 폴링·SSG. **agent_alpha 사이드카**
  ...
  스파크 강도색·큰 수 우선 정렬(`sparkClass`/`sparkRank`) + 키움 속 숨은 외국인 매집 밝은 오렌지 배지(`hiddenForeign`/`HF_BADGE`) — 위 `/alpha` API 항목 참조. 모두 웹 표시 전용(alpha.json·정렬 데이터 무수정).
