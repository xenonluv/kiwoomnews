...
[유니버스] 시장별(코스피/코스닥) 거래대금 TOP20(KIS volume-rank) + 등락률 TOP20(네이버 up 랭킹)
           합집합 → 등락률 밴드 −6~10%. KIS 장애 시 네이버 전수 스캔 폴백(params.universe로 구분)
   ▼
[정밀 판정·종목별, KIS 공식 API] price_now(고가+13% 전제, 2트랙: fade=식음 −6~+10% /
                                shakeout=분봉 고점 −10%+ 눌림 후 낙폭 30%+ 회복, ≤+30%)
                                → 일봉 MA10 → 당일 분봉 스파크
   ▼                              → 투자자 수급(외인/기관 순매수, 가점)
...
| `radar.py` | 6조건 스캐너 CLI. 유니버스 = 시장별 거래대금·등락률 TOP-N 합집합(`--top-n` 기본 20, KIS 장애 시 네이버 전수 스캔 자동 폴백). **2트랙 패턴**: fade(급등 후 식음, 기존) + shakeout(눌림 후 재상승 — `--shake-pct` 10, `--shake-recover` 0.3, `--shake-chg-max` 30; suspect에 `pattern`/`shake` 필드, 점수는 fade 슬롯 재사용으로 백테스트 호환). `--min-value`(원, 기본 700억 — **정밀판정 게이트**로만 적용) `--high-pct`(13) `--chg-min/max`(−6/10) `--spark-x`(분봉 거래량 중앙값 8배) `--spark-pct`(0.8) `--names`(watchlist). stdout JSON. 유니버스 0종목이면 exit 2 (수집 장애 구분). |
...
| `publish.py` | radar → `web/data/radar.json` → 변경 시에만 commit+push. flock 락, `--dry-run`(/tmp/radar_preview.json), `--max`(기본 12). radar 인자 그대로 전달. 매 회차 `data/radar_history/`에 검증용 이력 기록(raw 점수). |
| `radar_backtest.py` | **자가 검증·개선** (cron 17:20): 익일 일봉 대조(적중=익일종가>신호일종가, entry는 일봉 종가 재정합) → **당일 마감카드 AI 예측 기록**(`ai_predict` — 프로덕션 `/api/stock/{code}/ai` 호출, history에 `ai_pred{prob_up,direction}` 저장, `RADAR_AI_PREDICT=0`으로 비활성) → 점수대 보정표(n≥20) → 표본 n≥30 시 가중치 자동 튜닝(±30% bounded, `data/radar_weights.json`) → `web/data/performance.json`(대시보드, `ai` 블록 = AI 방향별 적중률·확률구간 보정·Brier) → `--push`. 통계는 **raw(가중치 적용 전) 점수**만 사용(드리프트 방지). 25일 초과 미평가는 만료 처리. |
...
- **KIS 공식 API** (`openapi.koreainvestment.com:9443`, .env의 KIS_APP_KEY/SECRET):
  ...
  실전 rate ~20건/초(클라이언트 0.06초 간격). 거래대금 순위 `FHPST01710000`(volume-rank,
  거래금액순 — TOP20 유니버스에 사용, `FID_INPUT_ISCD` 0001=코스피/1001=코스닥, ~30행).
  ⚠ 등락률 순위 `FHPST01700000`은 **정렬 코드 0~4 전부 등락률순으로 동작 안 함(실측)** →
...
20 17 * * 1-5      cd /home/xenonluv/stocknews && /usr/bin/python3 scripts/radar_backtest.py --push >> /tmp/radar_backtest.log 2>&1
```
(일괄 설치는 `bash scripts/install_cron.sh` — analyzer cron 포함, idempotent)
